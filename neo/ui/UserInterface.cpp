...
idCVar binaryLoadGuis( "binaryLoadGuis", "1", CVAR_GUI, "enable binary load/write of particle decls" );
...
void idUserInterfaceLocal::DrawCursor()
{
	...
	else
	{
		...
		if( !vrSystem->IsActive() || vr_controllerGamepad.GetInteger() == 1 || !vrSystem->VR_USE_MOTION_CONTROLS || vr_guiMode.GetInteger() != 2 || vr_debugTouchCursor.GetBool() ) //!gameLocal.GetLocalPlayer()->GuiActive()
		{

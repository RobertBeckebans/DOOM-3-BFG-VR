...
void idWeapon::UpdateVRGUI()
{
	...
	if( isPlayerFlashlight || isPlayerLeftHand )
	{
		return;
	}
	...
	if( player )
	{
		...
		if( healthv >= 75.0 )
		{
			healthb = ( healthv / 100.0f ) ;
		}
		if( armorv >= 75.0 )
		{
			armorb = ( armorv / 100.0f ) ;
		}
...
weapon_t idWeapon::IdentifyWeapon()
{
	...
	if( this )
	{
		if( weaponDef != NULL )
		{
			...
			if( idStr::Icmp( "weapon_fists", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_FISTS;
			}
			else if( idStr::Icmp( "weapon_chainsaw", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_CHAINSAW;
			}
			else if( idStr::Icmp( "weapon_pistol", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_PISTOL;
			}
			else if( idStr::Icmp( "weapon_shotgun", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_SHOTGUN;
			}
			else if( idStr::Icmp( "weapon_machinegun", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_MACHINEGUN;
			}
			else if( idStr::Icmp( "weapon_chaingun", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_CHAINGUN;
			}
			else if( idStr::Icmp( "weapon_handgrenade", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_HANDGRENADE;
			}
			else if( idStr::Icmp( "weapon_plasmagun", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_PLASMAGUN;
			}
			else if( idStr::Icmp( "weapon_rocketlauncher", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_ROCKETLAUNCHER;
			}
			else if( idStr::Icmp( "weapon_bfg", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_BFG;
			}
			else if( idStr::Icmp( "weapon_soulcube", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_SOULCUBE;
			}
			else if( idStr::Icmp( "weapon_shotgun_double_mp", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_SHOTGUN_DOUBLE_MP;
			}
			else if( idStr::Icmp( "weapon_grabber", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_GRABBER;
			}
			else if( idStr::Icmp( "weapon_shotgun_double", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_SHOTGUN_DOUBLE;
			}
			else if( idStr::Icmp( "weapon_artifact", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_ARTIFACT;
			}
			else if( idStr::Icmp( "weapon_bloodstone_passive", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_ARTIFACT;
			}
			else if( idStr::Icmp( "weapon_bloodstone_active1", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_ARTIFACT;
			}
			else if( idStr::Icmp( "weapon_bloodstone_active2", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_ARTIFACT;
			}
			else if( idStr::Icmp( "weapon_bloodstone_active3", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_ARTIFACT;
			}
			else if( idStr::Icmp( "weapon_pda", weaponDef->GetName() ) == 0 )
			{
				currentWeapon = WEAPON_PDA;
			}
			else if( idStr::Icmp( "weapon_flashlight_new", weaponDef->GetName() ) == 0 )
			{
	...
	if( currentWeapon == WEAPON_FLASHLIGHT )
	{
		common->Printf( "Identify weapon returned %s\n", weaponDef->GetName() );
	}

...
void idRenderBackend::DrawView( const void* data, const int stereoEye )
{
	...
	if( vrSystem->IsActive() && ( !cmd->viewDef->isSubview || cmd->viewDef->isMirror ) ) // dont fix up if we are projecting the cinematic into space or if this is a subview ( security cam etc. )
	{

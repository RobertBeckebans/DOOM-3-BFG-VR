...
void idRenderBackend::DrawViewInternal( const viewDef_t* _viewDef, const int stereoEye )
{
	...
	// render debug tools
	//-------------------------------------------------
	//DGB_RenderDebugTools( drawSurfs, numDrawSurfs );
...
void idRenderBackend::DrawView( const void* data, const int stereoEye )
{
	...
	if( vrSystem->IsActive() && ( !cmd->viewDef->isSubview || cmd->viewDef->isMirror ) && !( gameLocal.inCinematic && vr_cinematics.GetInteger() == 2 ) ) // dont fix up if we are projecting the cinematic into space or if this is a subview ( security cam etc. )
	{
